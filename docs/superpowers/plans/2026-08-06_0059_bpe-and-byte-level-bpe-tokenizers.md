...
internal sealed class BpePreTokenizer
{
    private static readonly Regex Whitespace =
        // Corrected in Task 7: this was @"\S+", which is HuggingFace's WhitespaceSplit,
        // not its Whitespace — the type tiny_bpe.json actually declares, and the one
        // WordPieceTokenizer already ships this exact regex for. Task 6's corpus never
        // exercised the null branch, so the wrong default shipped unverified.
...
**Corrections, found during implementation.** The Step 3 code above fails 11 of the 20 `bpe.json` cases, and the corpus is the authority. What shipped differs in three ways; later tasks read `BpeTokenizer.cs` itself, not this snapshot:

1. **The `[UNK]` rule is per code point, not per piece.** `InitialSymbols` emits one unknown id per uncovered *code point* and merging continues around it, instead of the whole piece collapsing to a single `[UNK]`. Case 12 pins it exactly: 👋🏽 costs 2 unknowns, the ZWJ family emoji 7, 🇫🇷 2 — a loop over `char` would give 4, 11 and 4. When the model declares no unknown token, the code point is dropped, matching `merge_word`.
2. **Added tokens are matched literally, before the pre-tokenizer runs** — HuggingFace's `AddedVocabulary` stage, which the plan omitted. Case 14 is arithmetically impossible without it: 11 expected ids against 15. The rule is leftmost, then longest at equal position, and `BpeTokenizerTests` asserts it directly because the corpus declares only one added token and so cannot. An added token whose content is empty is filtered at construction — `TokenizerJsonLoader.ReadAddedTokens` bounds the upper length but never rejects `""`, and a zero-length match makes the scan position stop advancing.
3. **A duplicate merge pair keeps its first rank, not its last.** `tiny_bpe.json` has 116 merges and 116 distinct pairs, so no corpus in this branch can tell the two apart; lowest-rank-wins is the BPE invariant and the code owns the choice rather than claiming to know what HuggingFace does.
