...
- [ ] **Step 4: Confirm it is reproducible**

Run the builder a second time and check nothing moved:

```bash
git diff --stat -- tests/oracles/tiny_bpe.json
```

Expected: empty. `tiny_bpe.json` is committed like `tiny_sp.model` — built once, not regenerated by the CI drift job. If the second run *does* differ, the trainer is not deterministic on this input; report that to the user before continuing, because the whole fixture strategy depends on it.
