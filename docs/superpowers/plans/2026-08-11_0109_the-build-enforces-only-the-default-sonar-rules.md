...
intersection — **nine rules, measured** — becomes severity entries in a root `.globalconfig`, which the SDK
discovers on its own; `Directory.Build.props` carries only the comment saying so. The same script in `--check` mode guards the file against profile drift on every
pull request. The second net is a pinned SonarQube Community container plus the honest list of what it does
not reproduce.

**Tech Stack:** Python 3.12 (`urllib`, no new runtime dependency), pytest (new, dev-only),
MSBuild's own `.globalconfig` discovery, SonarAnalyzer.CSharp 10.20.0.135146, SonarQube Community in Docker or Podman.
...
- Read the pass/fail **counts** of every test run. A `--filter` that matches nothing exits zero and reports
  ...
  **superseded during execution**: `origin/main` was merged in mid-branch, bringing issue #92's regression
  metrics, and the baseline from Task 3 onwards is **2915 passing, 0 failed**.
...
| `Directory.Build.props` | The comment saying why no item declares `.globalconfig`, and why adding one would silently switch it off. |
...
- [ ] **Step 2: Say, where a reader will look for the wiring, why there is none**

**Amended during execution, 2026-08-12.** This step originally added a
...
project's own directory. Applying it literally **turns the mechanism off**: the SDK already discovers a
root `.globalconfig` from any ancestor directory, so the item registers the same path twice, MSBuild logs
`MultipleGlobalAnalyzerKeys`, and every severity in the file is dropped — Step 3's `S107` canary goes
quiet. Measured independently by the implementer and by the reviewer on SDK 10.0.110.

What goes into the root `Directory.Build.props`, after the `PropertyGroup` that pins
`DataNetSonarAnalyzerVersion`, is therefore a comment and no item. It carries four things: that the file is
generated by `tools/generate_sonar_globalconfig.py` and hand edits to it are lost; that the SDK's own
discovery applies it with no wiring, the repository root being an ancestor of every project, `samples/`
included; that adding an `Include` back registers it twice and silently drops every severity, with the
canary as the evidence; and what whoever sets `DiscoverGlobalAnalyzerConfigFiles` to `false` would have to
write — next to the property that disabled it, not here.
...
`samples/` is outside the solution and is reached by the SDK's discovery rather than by anything this
branch declares, so this is the step that proves the file arrives there at all. The nine rules were measured to produce nothing there
either.
