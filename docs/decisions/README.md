...
| [`0050`](0050-the-sentencepiece-bpe-lineage-stays-a-bpe-model.md) | The SentencePiece-BPE lineage stays a BPE model, and metaspace becomes one transform | accepted | 2026-08-21 | Amended by [`0062`](0062-the-two-metaspace-spellings-are-one-value-but-for-the-prepend-guard.md), which bounds its §2: the two spellings are one value on every text that does not already begin with the replacement, and two on every text that does. Amends [`0017`](0017-bpe-parity-scope.md) §3, whose `byte_fallback` refusal and "no path here" for Llama-2 and Mistral v0.1 both fall |
...
| [`0062`](0062-the-two-metaspace-spellings-are-one-value-but-for-the-prepend-guard.md) | The two metaspace spellings are one value, but for the prepend guard | accepted | 2026-08-30 | Amends [`0050`](0050-the-sentencepiece-bpe-lineage-stays-a-bpe-model.md) §2, whose equality it bounds rather than drops: `MetaspaceEscape` carries which declaration it was read from, because `Metaspace` guards its prepend on `starts_with` and the `Prepend` + `Replace` sequence cannot. Refused shipping the divergence measured and documented, 0050 §4's rule having no weaker form for a file we accept |

## What `accepted` means here

All sixty-two carry `accepted`. None has been rejected or withdrawn — a status this table
...
of its own `## Done` section, and its status reads `accepted` like the other sixty-one.
...
[`0062`](0062-the-two-metaspace-spellings-are-one-value-but-for-the-prepend-guard.md) are the same
shape, and the reader most likely to need it is the one who opens 0050 to find out what the
metaspace transform is: 0050 §2 still says "two writings of one value", which is true of 48 of the
corpus's 54 pairs and false of the other six.
