...
| `Tokenizer(Unigram(...)).encode(t)` / `sp.encode(t)` | tokenizers / sentencepiece | `new SentencePieceTokenizer(vocab).Encode(t)` | Unigram par Viterbi (max de log-probabilités). Préfixe `▁`, normaliseur `identity`. Parité exacte. |
