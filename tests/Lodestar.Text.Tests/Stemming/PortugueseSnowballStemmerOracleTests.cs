...
public sealed class PortugueseSnowballStemmerOracleTests
{
    private static readonly OracleFile<PorterCase> Corpus = OracleCorpus.Load<PorterCase>("snowball_pt.json");
    ...
    public void Stem_matches_nltk()
    {
        OracleAsserts.ExactString(Corpus.Cases,
            c => c.Stem,
            c => PortugueseSnowballStemmer.Stem(c.Word),
            c => $"[#{c.Id}] \"{c.Word}\"");
    }
    ...
    {
        Assert.Throws<ArgumentNullException>(() => PortugueseSnowballStemmer.Stem(null!));
    }
}
