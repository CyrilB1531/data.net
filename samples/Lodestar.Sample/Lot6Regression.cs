...
/// <summary>
/// Lot 6 — Lodestar.Metrics, the regression half of the scikit-learn-compatible
...
internal static class Lot6Regression
{
    ...
    private static void Errors()
    {
        ...
        Console.WriteLine($"  RootMeanSquaredError  = {Inv.F3(RootMeanSquaredError.Score(Truth, Predicted))}");
        Console.WriteLine($"  MeanAbsoluteError     = {Inv.F3(MeanAbsoluteError.Score(Truth, Predicted))}");
        Console.WriteLine($"  MedianAbsoluteError   = {Inv.F3(MedianAbsoluteError.Score(Truth, Predicted))}");
        Console.WriteLine($"  MeanAbsolutePctError  = {Inv.F3(MeanAbsolutePercentageError.Score(Truth, Predicted))}");
        ...
        Console.WriteLine($"  MaxError              = {Inv.F3(MaxError.Score(Truth, Predicted))}");

        // alpha = 0.5 is half the mean absolute error, which is the identity that
        // ties the quantile loss to a metric already above it.
        Console.WriteLine($"  PinballLoss (α=0.5)   = {Inv.F3(PinballLoss.Score(Truth, Predicted))}"
            + $" — half of MAE, {Inv.F3(MeanAbsoluteError.Score(Truth, Predicted) / 2.0)}");
        Console.WriteLine($"  PinballLoss (α=0.9)   = {Inv.F3(PinballLoss.Score(Truth, Predicted, alpha: 0.9))}"
        ...
        Console.WriteLine($"  MedianAbsoluteError   = "
            + $"{Inv.F3(MedianAbsoluteError.Score(Truth, Predicted, sampleWeight: uniform))} under uniform weights");
        Console.WriteLine();
    }
    ...
    {
        Console.WriteLine($"  MeanSquaredLogError   = {Inv.F5(MeanSquaredLogError.Score(PositiveTruth, PositivePredicted))}");
        Console.WriteLine($"  RootMeanSquaredLogErr = {Inv.F5(RootMeanSquaredLogError.Score(PositiveTruth, PositivePredicted))}");
    ...
    private static void Explained()
    {
        ...
        Console.WriteLine($"  ExplainedVariance     = {Inv.F3(ExplainedVariance.Score(Truth, Predicted))}");
        ...
        Console.WriteLine($"  R2 on a flat truth    = {Inv.F3(R2.Score(flatTruth, exact))} perfect,"
            + $" {Inv.F3(R2.Score(flatTruth, wrong))} imperfect");
        Console.WriteLine($"  the same, unclamped   = {Inv.F3(R2.Score(flatTruth, exact, forceFinite: false))} perfect,"
            + $" {Inv.F3(R2.Score(flatTruth, wrong, forceFinite: false))} imperfect");
        ...
        double[] single = [3.0];
        double[] guess = [5.0];
        Console.WriteLine($"  R2 on one sample      = {Inv.F3(R2.Score(single, guess))}"
            + $" (ZeroDivision.Zero gives {Inv.F3(R2.Score(single, guess, zeroDivision: ZeroDivision.Zero))})");
        ...
        Console.WriteLine($"  ExplainedVariance     = {Inv.F3(ExplainedVariance.Score(single, guess))} on that same sample");
        Console.WriteLine();
    }
    ...
    private static void MultiOutput()
    {
        ...
        Console.WriteLine($"    MSE uniform average = {Inv.F3(MeanSquaredError.Score(WideTruth, WidePredicted, outputCount: 2))}");
        Console.WriteLine($"    MSE raw values      = {Inv.List(MeanSquaredError.PerOutput(WideTruth, WidePredicted, outputCount: 2))}");
        ...
        Console.WriteLine($"    MSE weighted [.3,.7]= "
            + $"{Inv.F3(MeanSquaredError.Score(WideTruth, WidePredicted, outputCount: 2, outputWeights: outputWeights))}");

        Console.WriteLine($"    MAE raw values      = {Inv.List(MeanAbsoluteError.PerOutput(WideTruth, WidePredicted, outputCount: 2))}");
        Console.WriteLine($"    MedianAE raw values = {Inv.List(MedianAbsoluteError.PerOutput(WideTruth, WidePredicted, outputCount: 2))}");
        Console.WriteLine($"    MAPE raw values     = {Inv.List(MeanAbsolutePercentageError.PerOutput(WideTruth, WidePredicted, outputCount: 2))}");
        Console.WriteLine($"    Pinball raw values  = {Inv.List(PinballLoss.PerOutput(WideTruth, WidePredicted, alpha: 0.9, outputCount: 2))}");
        Console.WriteLine($"    RMSE raw values     = {Inv.List(RootMeanSquaredError.PerOutput(WideTruth, WidePredicted, outputCount: 2))}");
